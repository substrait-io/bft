...
def normalize_data(data):
    ...
    if isinstance(data, dict):
        ...
        # Remove single or double quotes around the string
        #data = re.sub(r"^['\"](.*)['\"]$", r"\1", data).strip()
...
def compare_yaml_files(file1, file2):
    ...
    diff = DeepDiff(data1, data2, ignore_order=True, ignore_numeric_type_changes=True, view='text')
    if diff:
    ...
    for root, _, files in os.walk(original_dir):
        for file_name in files:
            if file_name.endswith('.yaml'):
                ...
                roundtrip_file = os.path.join(roundtrip_dir, relative_path).replace('.test', '.yaml')
...
def main():
    ...
    if count == 0:
        ...
        print(f"Differences found in {count} YAML files between original and roundtrip directories.")
