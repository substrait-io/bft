...
from convert_tests_helper import convert_to_new_value
...
def generate_define_table(case, table_id):
    ...
    table_definition = f"DEFINE t{table_id}({formatted_columns}) = ({formatted_rows}) \n"
...
def format_test_case(case, function, description_map, table_id_counter, is_aggregate):
    ...
    options = f" [{', '.join(f'{k}:{convert_to_new_value(v, None)}' for k, v in options.items())}]" if options else ''
    ...
    args = ', '.join(convert_to_new_value(arg.get('value'), str(arg['type'])) for arg in case.get('args', []))
    return description, f"{function}({args}){options} = {results}"


def parse_test_data(input_data, prefix, is_aggregate):
    ...
    for case in input_data['cases']:
        description, formatted_test = format_test_case(case, function, description_map, table_id_counter, is_aggregate)
        groups[description]['tests'].append(formatted_test)

    return groups, base_uri

def output_test_data(test_groups, base_uri, is_aggregate, output_file):
    """Main function to write formatted test cases to output file."""
    output_lines = [
        f"{'### SUBSTRAIT_AGGREGATE_TEST: v1.0' if is_aggregate else '### SUBSTRAIT_SCALAR_TEST: v1.0'}\n",
        f"### SUBSTRAIT_INCLUDE: '{base_uri}'\n"
    ]

    for description, details in test_groups.items():
...
def convert_directory(input_dir, output_dir, prefix):
    ...
    for root, _, files in os.walk(input_dir):
        for filename in filter(lambda f: f.endswith('.yaml'), files):
            ...
            output_file = os.path.join(output_dir, os.path.relpath(input_file, input_dir)).replace('.yaml', '.test')
            is_aggregate = 'aggregate' in input_file

            yaml_data = load_test_file(input_file)
            parsed_tests, base_uri = parse_test_data(yaml_data, prefix, is_aggregate)
            output_test_data(parsed_tests, base_uri, is_aggregate, output_file)


if __name__ == '__main__':
    input_directory = './cases'
    output_directory = './substrait/tests/cases'
    uri_prefix = 'https://github.com/substrait-io/substrait/blob/main/extensions/substrait'
