...
from tests.coverage.nodes import TestFile, AggregateArgument
from tests.coverage.case_file_parser import load_all_testcases
from convert_tests_helper import convert_to_old_value, convert_to_long_type, SQUOTE, DQUOTE, iso_duration_to_timedelta
...
def convert_result(test_case):
    ...
    if test_case.is_return_type_error():
        ...
        return {
            'value': convert_to_old_value(iso_duration_to_timedelta(test_case.result.value), 'str'),
            'type': 'string'
        }
    else:
        return {
            'value': convert_to_old_value(test_case.result.value, test_case.result.type),
...
def convert_table_definition(test_case):
    ...
    if test_case.column_types is not None:
        ...
        column_types = [
            convert_to_long_type(arg.scalar_value.type if isinstance(arg, AggregateArgument) else arg.type)
    ...
    if not columns:
        ...
        return [
            {'value': convert_to_old_value(column, col_type), 'type': col_type,
...
def convert_test_case(test_case, groups):
    ...
    if test_case.rows is not None:
        ...
        if isinstance(test_case.args[0], AggregateArgument):
            case['args'] = [
                {'value': convert_to_old_value(arg.scalar_value.value, arg.scalar_value.type),
                 'type': convert_to_long_type(arg.scalar_value.type)} for arg in test_case.args
            ]
        else:
            case['args'] = [
                {'value': convert_to_old_value(arg.value, arg.type), 'type': convert_to_long_type(arg.type)} for arg in
                test_case.args
            ]

    if len(test_case.options) > 0:
        case['options'] = {key: convert_to_old_value(value, None) for key, value in test_case.options.items()}

    case['result'] = convert_result(test_case)
    return case

def convert_test_file_to_yaml(testFile: TestFile, output_file):
    ...
    for test_case in testFile.testcases:
        ...
        cases.append(convert_test_case(test_case, groups))
    ...
    print(f"Converted '{testFile.path}' to '{output_file}'.")
...
def fix_quotes(file_path):
    ...
    content = content.replace("'", "").replace('"', '').replace(SQUOTE, "'").replace(DQUOTE, '"')
...
def convert_directory(input_dir, output_dir):
    ...
    for input_test_file in input_test_files:
        ...
        convert_test_file_to_yaml(input_test_file, output_file)
